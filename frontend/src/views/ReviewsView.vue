            ...
            <table class="w-full text-sm">
              ...
              <tbody class="divide-y divide-slate-100 dark:divide-[#2a3347]">
                <tr v-for="g in groups" :key="g.id">
                  ...
                  <template v-for="r in roster" :key="r">
                    ...
                    <td
                      ...
                      class="px-2 py-1.5 text-center font-mono font-semibold text-red-800 dark:text-red-400
                             bg-red-300 dark:bg-red-900/30 border-l border-slate-200 dark:border-[#2a3347]"
                      title="自己指導的組別，不可評分"
                    >—</td>
                    <template v-else>
                      <!-- 輸入框填滿整格，所以點格子任何位置都會進入編輯 -->
                      <td
                        v-for="(sc, i) in subCols" :key="sc.key"
                      ...
                      >
                        ...
                        <input
                          ...
                          class="input px-1 py-0.5 text-xs w-full min-w-[7rem]"
                          @blur="commitCell(g, r)"
                        />
                        <span
                        ...
                        <input
                          ...
                          class="input px-1 py-0.5 text-sm text-center w-full min-w-[3.5rem]"
                          @blur="commitCell(g, r)"
                ...
                <tr v-else-if="!groups.length">
                  <td :colspan="3 + roster.length * subCols.length + 3"
